#!/usr/bin/env python3
"""
extract_muts-CPTAC.py

Extracts patient-level mutation features from CPTAC GDC MAF files.

This program does NOT match mutations to UNI2-h embeddings. It only:

1. Reads CPTAC .maf.gz files listed in a GDC manifest.
2. Extracts mutation records.
3. Keeps qualifying non-silent mutations by default.
4. Combines multiple MAF files belonging to the same case_id.
5. Builds a sparse binary patient-mutation feature matrix.
6. Queries GDC metadata to assign an organ-level tumor_origin.
7. Saves patient metadata, QC information, and an audit table.

COLUMN NAMING (IMPORTANT -- fixed from the original version of this script)
    The real, current TCGA-side mutation extraction script
    (final_extract_muts.py) produces columns prefixed "som_mut_" (e.g.
    "som_mut_TP53_any", "som_mut_TP53_Missense_Mutation") -- confirmed
    directly against its actual current source. The original version of
    this script produced BARE column names ("TP53", "TP53_Missense_
    Mutation") with no prefix at all, which would never match the real
    training columns during alignment in validate_CPTAC.py -- every
    mutation feature from this script would have been silently treated
    as "not present in the training column list" and excluded.

    Additionally, the original version generated a third feature type,
    "<Gene>_HGVSp" (presence of a recorded protein-level annotation).
    This feature type was REMOVED from the TCGA training pipeline
    (final_extract_muts.py) after direct analysis showed HGVSp is
    present in >99% of rows for every retained non-silent mutation
    class, making it almost entirely redundant with the broader
    "<Gene>_any" indicator. Since no "som_mut_<Gene>_HGVSp" column
    exists in the training column space anymore, generating it here
    would just be wasted computation (it can never match anything
    during alignment) -- removed for consistency with the training side.

PATIENT MUTATION FEATURE DESIGN (current)
------------------------------------------

For every gene observed in the qualifying mutation set, TWO binary
feature categories are generated, matching final_extract_muts.py:

    som_mut_TP53_any
        1 if the patient has any qualifying TP53 mutation.

    som_mut_TP53_Missense_Mutation
        1 if the patient has a TP53 missense mutation.

All features are binary presence/absence indicators.

Variant_Type, read depths, and VAF are retained in mutations_long for
auditing but are not used as matrix features.

TUMOR ORIGIN
------------

Tumor origin is an organ-level target:
...
The original GDC project_id, disease_type, and primary_site are also
preserved in patient_metadata.csv.
...
------------
    ...
    mutations_long.parquet
      or mutations_long.csv if Parquet support is unavailable
...
----------------
    ...
    tumor_origin<TAB>case_id

This is compatible with the rows-file convention used by the SBS96
extraction program.
...
------------
    ...
    pip install --break-system-packages \
        pandas numpy scipy tqdm requests pyarrow

USAGE
-----

    python3 /workspace/extract_muts-CPTAC.py \
        --maf-dir /workspace/cptac_maf_downloads \
        --manifest /workspace/CPTAC-MAFS-Manifest.txt \
        --output-dir /workspace/CPTAC_mutation_tables
"""
...
# real prefix as baked in by final_extract_muts.py -- confirmed directly
# against its actual current source
...
# ---------------------------------------------------------------------
# Mutation filtering and feature generation
# ---------------------------------------------------------------------
...
def mutation_features_for_patient(
    ...
    """
    Two feature types per gene, matching final_extract_muts.py's real
    current column-naming convention exactly:
        som_mut_<Gene>_any               -- any qualifying mutation
        som_mut_<Gene>_<Classification>  -- that specific classification

    A third feature type (HGVSp presence) existed in the original
    version of this script but has been removed here, since it no
    longer exists in the training column space (removed from
    final_extract_muts.py after being found >99% redundant with the
    broad "_any" indicator).
    """
    features = set()
    ...
    for _, row in patient_df.iterrows():
        ...
        # Any qualifying mutation in the gene.
        features.add(f"{MUTATION_PREFIX}{gene}_any")

        # Specific mutation-classification presence.
        if classification:
            features.add(
...
def main() -> None:
    ...
    for maf_path in tqdm(
        ...
        try:
            ...
            # Use submitter barcode metadata when needed.
            if (
                not metadata_record
