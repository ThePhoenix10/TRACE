#!/usr/bin/env python3
"""
extract_cna_gistic.py

Builds a binary per-gene copy number alteration (CNA) feature matrix, one
row per patient, from the pre-computed TCGA PanCanAtlas GISTIC2
"thresholded by genes" file -- NOT from the raw ASCAT3 downloads. Output
is in the SAME three-file format as final_extract_muts.py's mutation
matrix and extract_mutational_signatures.py's signature matrix, so it
plugs straight into fuse_tables.py alongside them.

INPUT FILE
TCGA.PANCAN.sampleMap/Gistic2_CopyNumber_Gistic2_all_thresholded.by_genes
from UCSC Xena: genes as rows, TCGA sample barcodes as columns, values:
    -2  deep deletion
    -1  shallow deletion / loss
     0  normal / no alteration
     1  gain
     2  amplification

FEATURE NAMING
For every non-neutral CNA, the program emits TWO binary features:

    cna_<GENE>_any      broad flag: any CNA in this gene
    cna_<GENE>_<state>  specific state flag, where <state> is one of:
                        del, loss, gain, amp

Examples:
    ERBB2 amplification -> cna_ERBB2_any = 1 and cna_ERBB2_amp = 1
    CDKN2A deep deletion -> cna_CDKN2A_any = 1 and cna_CDKN2A_del = 1

Neutral genes (value 0) emit no CNA feature.

IDENTITY BRIDGING
Sample barcodes are truncated to the first 12 characters before bridging
through case_id_barcode_map.csv and matched_cohort.csv.

USAGE
python extract_cna_gistic.py \
  --gistic-file /workspace/TCGA_PANCAN_sampleMap/Gistic2_CopyNumber_Gistic2_all_thresholded.by_genes.gz \
...
OUTPUT
    patient_cna_matrix.npz
    patient_cna_matrix_rows.txt
    patient_cna_matrix_columns.txt
    cna_extraction_summary.txt
"""
...
def get_alteration_flags(gene_name: str, value: float) -> list[str]:
    """
    Convert one thresholded GISTIC2 state into binary CNA features.

      -2 -> cna_<GENE>_any + cna_<GENE>_del
      -1 -> cna_<GENE>_any + cna_<GENE>_loss
       0 -> no features
      +1 -> cna_<GENE>_any + cna_<GENE>_gain
      +2 -> cna_<GENE>_any + cna_<GENE>_amp
    """
    if pd.isna(value):
        return []
...
def main():
    ...
    numeric_records = []  # (case_id, column) pairs; presence only
