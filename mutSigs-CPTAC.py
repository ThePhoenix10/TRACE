...
Extracts binary SBS96 trinucleotide-context features from CPTAC GDC MAF files.

Each patient is represented by 96 binary features:

    1 = the patient has at least one SNV in that SBS96 channel
    0 = the patient has no SNV in that SBS96 channel

Examples of SBS96 channels:
...
1. Reads CPTAC .maf.gz files downloaded with gdc-client.
2. Restricts processing to file IDs listed in the CPTAC manifest.
3. Uses only single-nucleotide substitutions.
4. Retrieves the trinucleotide sequence from a GRCh38 reference FASTA.
5. Normalizes substitutions to the pyrimidine C/T convention.
6. Combines multiple MAF files belonging to the same patient.
7. Produces a sparse patient-by-96 binary feature matrix.
8. Produces row labels, column labels, QC statistics, and a summary.

IMPORTANT
---------
This produces SBS96 mutation-context features. It does not fit COSMIC
signature exposures such as SBS1, SBS4, SBS5, or SBS7.

Required packages:

    pip install --break-system-packages pyfaidx pandas numpy scipy tqdm

Example:

    python3 /workspace/mutSigs-CPTAC.py \
        --maf-dir /workspace/cptac_maf_downloads \
        --manifest /workspace/CPTAC-MAFS-Manifest.txt \
        --reference-fasta /workspace/hg38.fa \
        --output-dir /workspace/CPTAC_SBS96

Optional cohort mapping CSV:

    --cohort-map-csv /workspace/cptac_case_cohort_mapping.csv

The optional mapping CSV may contain:

    case_id,tumor_origin

or:

    case_id,cohort

If no mapping CSV is supplied, the tumor-origin field in the row-label
file will remain blank.
"""
...
def detect_maf_header_line(maf_path: Path) -> int:
    """
    Find the first non-comment line in a gzipped MAF file.

    The returned value is the zero-based line number that pandas should
    use as the header after skipping leading comment lines.
    """
...
def find_maf_files(
    ...
    """
    Match downloaded MAF files to manifest file IDs.
...
def get_case_id_from_group(
    ...
    """
    Determine the patient identifier for a group of mutations.

    Preference:

    1. case_id
    2. Tumor_Sample_Barcode
    3. GDC file UUID as a fallback
    """
...
def get_patient_groups(
    ...
    """
    Split one MAF into patient groups.

    Most GDC MAF files contain one case, but this also safely handles a
    file containing multiple case IDs.
    """
...
def chromosome_candidates(chromosome: str) -> List[str]:
    """
    Generate possible FASTA chromosome names.
...
def fetch_trinucleotide(
    ...
    """
    Retrieve the reference trinucleotide centered on a one-based MAF
    position.
    """
...
def get_sbs96_channel(
    ...
    """
    Convert one SNV into its pyrimidine-normalized SBS96 channel.
    """
...
def process_patient_mutations(
    ...
    """
    Build the set of SBS96 channels present in one patient and return
    detailed QC counts.
    """
